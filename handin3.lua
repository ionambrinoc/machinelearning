...
-- as we can see in the first image enclosed, training error converges downwards towards 0, but test
-- error is massive (the graph given is scaled down); this is an obvious case of overfitting.

--**************************************************************************************************--
-- Better optimization:

--++Running LBFGS with minibatch size 500 on training set of 40000 gives the best results I have seen;
--it is pretty quick to converge (~7 epochs), to a training error of ~300 & test error ~3.12; going
--up to 15 epochs does not help too much, since convergence has been reached.
...
--a 2nd order method, LBFGS is pretty quick to process and converge. (see second enclosed image)
