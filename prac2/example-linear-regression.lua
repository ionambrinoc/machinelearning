...
data = torch.Tensor{
   {40,  6,  4},   -- f(6,4)=40 f(10,4)=44 f(x,y)=a+bx+cy; a,b,c=?
   {44, 10,  4},
   {46, 12,  5}, --   x1
   {48, 14,  7}, --    \
   {52, 16,  9}, --     \
   {58, 18, 12}, --   1-->[ f ]---> f(1,x,x2)
   {60, 22, 14}, --     /
   {68, 24, 20}, --    /
   {74, 26, 21}, --   /
   {80, 32, 24}  --  x2
...
feval = function(x_new) --eval loss function & grad
   ...
   -- select a new training sample -- a row of "data"
   ...
   -- return loss(x) and dloss/dx (updated by backpropagation)
   return loss_x, dl_dx
end
...
sgd_params = {
   learningRate = 1e-3,   --10^(-3)
   learningRateDecay = 1e-4, --10^(-4)
   weightDecay = 0,
   momentum = 0
}
...
-- we cycle 1e4=10^4 times over our training data
for i = 1,1e3 do
   ...
   for i = 1,(#data)[1] do
      ...
      --     given a point x
      --   + a point x - weight vector
      --   + some parameters, which are algorithm-specific
      
      params,fs = optim.sgd(feval,x,sgd_params)
...
for i=1,(#dataTest)[1] do
   local prediction = model:forward(dataTest[i][{{1,2}}])
   print(string.format("%2d  %6.2f", i, prediction[1]))
end

-- predictionsadd column to tensor
-- 1e4 epochs    1e3 epochs    1e5 epochs
-- 1   40.10	  33.37		40.33
-- 2   43.88	  40.44		44.03
-- 3   49.89	  47.06		49.96

-- parameters:  bias insecticide  fertilizer
-- 1e3 epochs: 23.21 1.8392 	  -0.24
-- 1e4 epochs: 31.63 1.1146  	   0.6676
-- 1e5 epochs: 31.98 1.11    	   0.64

----------------------------------------------------------------------
-- Q3. Least squares solution
-- not optimization, simply use formula & data provided



y = data:narrow(2,1,1) --slice first column
X = torch.cat(torch.ones((#data)[1],1),data:narrow(2,2,2)) --slice rest of it
							--and add bias
XT=X:transpose(1,2)    --transpose X by swapping width (1) & height (2)

biasedDataTest = torch.cat(torch.ones((#dataTest)[1],1), dataTest)
print(biasedDataTest)

print('LSParams')
LSParams = (torch.inverse(XT * X))*XT*y
print(LSParams)

LSPredictions = biasedDataTest * LSParams
print(LSPredictions)
-- outputs:		parameters: 31.98  0.65  1.11 //closed form soln
-- 1. 40.32
-- 2. 44.03
-- 3. 49.96 
