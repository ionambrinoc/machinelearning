...
--------------------------------------------------------------------------------
-- Simple example of minimizing a function, f(x) = 1/2 x^2 + x sin(x) with 
-- derivative f'(x) = x + x cos(x) + sin(x).
...
-- preallocate space for gradient, which is 1-dim and 1 element since f is univariate
local grad = torch.Tensor{0}

-- return function's value and the gradient, at a given point x_vec
local function feval(x_vec)
    -- note: x_vec is a Tensor of 1-dim and size 1, so we get its one and only element:
    local x = x_vec[1]

    -- compute and return func val (scalar), and gradient (Tensor)
    f = 0.5*x^2 + x*torch.sin(x)
    grad[1] = x + torch.sin(x) + x*torch.cos(x)
    return f, grad -- f itself is the loss function; we return it and its gradient
end			-- for a given data point x_vec
